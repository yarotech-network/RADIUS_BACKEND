...
| `VITE_FEATURED_STOREFRONT_SLUG` | _(empty)_               | Slug of the storefront whose internet plans are showcased on the public landing page (`/`); empty hides the customer plans section.                              |
...
| 12    | ✅     | Tenant email verification with OTP (register → emailed 6-digit code → `POST /auth/verify-email/` signs the owner straight in; login blocked until verified), public landing page at `/` (hero, featured storefront plans, business/subscription plans, about) with the workspace overview moving to `/dashboard`, and a two-sided auth design (`AuthSplitLayout`) for login/agent login/register/verify. Backend: `apps/accounts/verification.py` + `EmailVerificationCode` model + `email_verified_at` on users (internal accounts verified by default, existing users backfilled). |
